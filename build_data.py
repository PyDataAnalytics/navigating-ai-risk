...
def paper_for(entry, pl, excerpts):
    ...
    return {
        ...
        "pdf": entry.get("oa_pdf_url")
        or entry.get("pdf_url")
        or "",  # full-text PDF (may download)
        "excerpt": excerpts.get(
            entry.get("key")
        ),  # {summary, key_findings, risk_relevance, ...} or None
...
def main():
    ...
    if not taxonomy:
        raise SystemExit(
            "corpus has no `taxonomy` block - regenerate it with the updated merge_corpus.py"
        )
    ...
    for entry in papers:
        for pl in entry.get("placements", []):
            bucket[(pl.get("category_id"), pl.get("subcategory_name"))].append(
                paper_for(entry, pl, excerpts)
            )
    ...
    ids = [c for c in CATEGORY_ORDER if c in cat_subs] + [
        c for c in first_order if c not in CATEGORY_ORDER
    ]
    categories = []
    for cid in ids:
        subs = cat_subs[cid]
    ...
    def fmt(iso):
        try:
            return datetime.datetime.fromisoformat((iso or "").replace("Z", "+00:00")).strftime(
                "%d %B %Y"
            )
        except Exception:
            return iso or ""
    ...
    print(
        f"  {len(categories)} categories | {total_subs} subcategories ({covered} covered, {total_subs - covered} empty)"
    )
